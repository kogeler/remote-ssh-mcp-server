...
.PHONY: help venv freeze refresh-dependencies freeze-check format lint \
	typecheck bandit audit test syntax shellcheck check ci clean \
...
lint: venv
	@$(RUFF) check remote_ssh_mcp tests remote-ssh-mcp.py
...
syntax: venv
	...
	@bash -n $(LAUNCHER) $(LIVE_HARNESS) $(LIVE_FIDO_HARNESS) $(LIVE_CORE)
