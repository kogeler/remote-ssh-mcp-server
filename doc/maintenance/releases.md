...
On a direct push to `main`, the release workflow first determines whether that
commit already has an exact release. A new release reuses the complete CI
workflow. Only the final publication job receives `contents: write`; it creates
or validates one exact-version tag and non-draft, non-prerelease release.
